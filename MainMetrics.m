...
if nc == 2
    ...
    [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_kw(1:150))),gndTrain,zscore(TestingSet(:, fea_kw(1:150))),gndTest, option1);
    result_KW = [acc, pre, rec, fs, area ];
    [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_releiff(1:150))),gndTrain,zscore(TestingSet(:, fea_releiff(1:150))),gndTest, option1);
    result_Relieff = [acc, pre, rec, fs, area ];
    [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_dmmls(1:150))),gndTrain,zscore(TestingSet(:, fea_dmmls(1:150))),gndTest, option1);
    result_ERLMS = [acc, pre, rec, fs, area];
    [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_sp(1:150))),gndTrain,zscore(TestingSet(:, fea_sp(1:150))),gndTest, option1);
    result_SP = [acc, pre, rec, fs, area ];
    ...
    for i = 1:nc
        ...
        [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_kw(1:150))),newTrainingLabel,zscore(TestingSet(:, fea_kw(1:150))),newTestingLabel, option1);
        result_KW1 = [acc, pre, rec, fs, area ];
        [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_relieff(1:150))),newTrainingLabel,zscore(TestingSet(:, fea_relieff(1:150))),newTestingLabel, option1);
        result_Relieff1 = [acc, pre, rec, fs, area ];
        [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_dmmls(1:150))),newTrainingLabel,zscore(TestingSet(:, fea_dmmls(1:150))),newTestingLabel, option1);
        result_ERLMS1 = [acc, pre, rec, fs, area];
        [acc, pre, rec, fs, area] = PredictCrossDatasetWithCG(zscore(TrainingSet(:, fea_sp(1:150))),newTrainingLabel,zscore(TestingSet(:, fea_sp(1:150))),newTestingLabel, option1);
        result_SP1 = [acc, pre, rec, fs, area ];
...
switch Dataset 
    case {1}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmprostatemetric.mat');
        else
            save('.\results\knnprostate.mat');
        end
    case {2}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmAMLALLmetric.mat');
        else
            save('.\results\knnAMLALL.mat');
        end
    case {3}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmDLBCLmetric.mat');
        else
            save('.\results\knnDLBCL.mat');
        end
    case {4}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmlungmetric.mat');
        else
            save('.\results\knnlung.mat');
        end
    case {5}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmMLLmetric.mat');
        else
    ...
    case {6}
        if strcmpi(option1.classifier, 'svm')
            save('.\results\svmSRBCTmetric.mat');
        else
